...
from utils import process_video, read_tarfile, convert_bounding_box_to_rectangle, convert_bounding_box_to_ellipse, pil_to_base64_jpg
...
class MADEval_FrameLoader():
    ...
    def __getitem__(self, index):
        ...
        # Encode to base64 images for gpt
        base64_images = []
        ...
        if char_text == ". Possible characters (labeled by {label_type}): ": # no character recognised
            text_prompt = self.general_prompt.format(video_type=self.video_type, char_text="", label_type=self.label_type, duration=str(round(end_-start_, 2)))
        else:
            text_prompt = self.general_prompt.format(video_type=self.video_type, char_text= char_text.format(label_type=self.label_type), label_type=self.label_type, duration=str(round(end_-start_, 2)))


        return_dict =  {'video': base64_images, \
                'imdbid': imdbid, \
                'prompt': text_prompt,
                'gt_text': gt_text,
                'start': start, 
